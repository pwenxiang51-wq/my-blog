...
3. 点击 添加记录 (Add record)：
   ...
   * 类型 (Type)：选 A。
   * 名称 (Name)：填个前缀，比如 vps (这样完整域名就是 vps.222382.xyz)。
   * 内容 (Content)：填你的 VPS IP 地址。
...
1. 输入 1 选择“安装/更新”。
2. 选择协议：推荐 Hysteria2 (速度快) 或 Vless Reality (稳定)。
3. 绑定域名：安装过程中，脚本通常会问你“是否拥有域名”或“是否申请证书”。
