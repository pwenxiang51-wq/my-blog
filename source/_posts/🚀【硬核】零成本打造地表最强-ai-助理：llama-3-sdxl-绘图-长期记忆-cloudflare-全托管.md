---
title: 🚀【硬核】零成本打造地表最强 AI 助理：Llama 3 + SDXL 绘图 + 长期记忆 (Cloudflare 全托管)
date: 2026-01-10T18:01:00.000+08:00
comments: true
---

> **前言**：在这个 AI 泛滥的时代，拥有一个完全属于自己的、隐私的、免费的、且功能强大的 AI 助理是每个极客的梦想。
>
> 本文将教你如何利用 **Cloudflare Workers** 的边缘计算能力，缝合 **Groq (Llama 3.3)** 的极速推理、**Workers AI (Stable Diffusion XL)** 的绘图能力，以及 **KV** 数据库的记忆能力，打造一个**“图文双修”**的 Telegram 机器人。

### 🌟 核心卖点

* 💸 **0 成本**：全部利用厂商免费额度（Cloudflare + Groq）。
* ⚡ **极速**：部署在边缘节点，全球毫秒级响应。
* 🎨 **多模态**：支持对话、写代码、生成 8K 电影级写实图片。
* 🧠 **有记忆**：记得住你是谁，支持连续对话（基于 KV 数据库）。
* 🛡️ **永不失联**：内置灾难恢复方案，即使 Groq 挂了也能切换 CF 原生 AI。
...
1.  **Cloudflare 账号**：用于托管代码和数据库（无需绑定信用卡）。
2.  **Telegram Bot Token**：
    * 在 TG 找 `@BotFather`，发送 `/newbot`，获取 Token。
    * 同时获取你自己的 TG ID (找 `@userinfobot` 获取)，作为管理员 ID。
3.  **Groq API Key**：
    * 去 [Groq Console](https://console.groq.com/) 免费申请一个 Key（目前业界速度最快的 Llama 推理服务）。
...
#### 1. 创建 KV 数据库 (用于存储记忆)
1.  登录 Cloudflare Dash，进入左侧 **Storage & Databases** -> **KV**。
2.  点击 **Create Namespace**，命名为 `AI_MEMORY`。
3.  点击 **Add**。

#### 2. 创建 Worker
1.  进入 **Compute (Workers)** -> **Create Worker**。
2.  命名为 `velo-ai-bot` (或者你喜欢的名字)，点击 **Deploy**。
3.  进入 Worker 的 **Settings** -> **Variables**，添加以下环境变量：
...
#### 3. 关键绑定 (Bindings)
**⚠️ 这一步最重要，错一个都跑不通！**
进入 Worker 的 **Settings** -> **Bindings**，点击 **Add**：
...
* **画图模式**：发送 `/img 关键词`（例如：`/img 赛博朋克黑客`）。系统会自动注入“写实、8K”等提示词，生成照片级大图。
...
**问：如果未来有一天，Groq 不再免费，或者挂了，我的机器人会死吗？**
**答：不会！**

Cloudflare 本身就内置了免费的 Llama 模型。如果 Groq 失效，你只需要修改代码中的**一个函数**，就能让机器人原地复活，切换回 Cloudflare 的内生动力。

**Plan B 代码：**
找到代码中的 `fetchGroqWithHistory` 函数，替换为以下内容即可：
...
* **项目源码**：[Velox-groqAI-Bot (v5.0 终极版)](https://github.com/pwenxiang51-wq/Velox-groqAI-Bot)
* **反馈与交流**：[点击进入 GitHub Discussions 参与讨论](https://github.com/pwenxiang51-wq/Velox-groqAI-Bot/discussions)
